...
class InstagramOAuthStrategyTest {
    ...
    void testExchangeCodeForTokens_NetworkError() {
        ...
        assertEquals("instagram", exception.getPlatform());
    ...
    void testGetUserPages_MinimalUserResponse() throws Exception {
        String accessToken = "test-access-token";
        String minimalResponseJson = "{\"id\":\"123456789\"}";
        ...
        when(mockJsonNode.get("id")).thenReturn(mockIdNode);
        when(mockIdNode.asText()).thenReturn("123456789");
        when(mockJsonNode.has("username")).thenReturn(false);
        when(mockJsonNode.has("account_type")).thenReturn(false);
