...
public class PageManagementController {
    ...
    @GetMapping("/reconnect/{platform}")
    public RedirectView reconnectPage(@PathVariable String platform, @RequestHeader("Authorization") String token) {
        try {
            Seller seller = tokenV2Repository.findSellerByToken(token).orElse(null);
            if (seller == null) {
                return new RedirectView(prefix + "/error?message=unauthorized");
            }
            ...
            e.printStackTrace();
            return new RedirectView(prefix + "/error?message=reconnect_failed");
        }
    }
}
