...
export class ClientSignupComponent {
  ...
  signup() {
    ...
    createUserWithEmailAndPassword(this.auth, email, password)
      ...
      .then(idToken => this.http.post(`${javaHost}/api/customer/auth/signup`, { idToken, name, username }).toPromise())
      .then((response: any) => {
  ...
  signupWithGoogle() {
    signInWithPopup(this.auth, new GoogleAuthProvider())
      ...
      .then(idToken => {
        ...
        return this.http.post(`${javaHost}/api/customer/auth/signup`, { idToken, name, username }).toPromise();
      })
      .then((response: any) => {
        // Store token in localStorage
        localStorage.setItem('token',"Bearer " +  response.token);
